...
def test_chat_stream_returns_confirmation_chunk_when_require_confirmation_enabled(
    ...
    monkeypatch.setattr(
        "n4ughtyllm_gate.adapters.openai_compat.router.settings.require_confirmation_on_block",
        True,
    )
    ...
    assert "Security interception notice" in text
    assert "Event ID: cfm-" in text
    assert "data: [DONE]" in text
